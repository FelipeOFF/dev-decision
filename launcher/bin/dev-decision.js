...
if (!["darwin", "linux"].includes(platform())) {
  console.error("Dev Decision suporta macOS e Linux.");
  process.exit(1);
}

const python = process.env.DEV_DECISION_BOOTSTRAP_PYTHON || "python3";
const root = join(homedir(), ".local", "share", "dev-decision");
...
if (existsSync(runtime) && installedVersion !== version && !update) {
  console.error(`Dev Decision ${installedVersion || "unknown"} is installed; run update explicitly to install ${version}.`);
  process.exit(2);
}
...
if (!existsSync(runtime) || update) {
  ...
  try {
    ...
    prepare(venv);
    required(runtime, ["-m", "dev_decision.public_cli", ...cliArgs]);
  ...
  const result = run(runtime, ["-m", "dev_decision.public_cli", ...cliArgs]);
