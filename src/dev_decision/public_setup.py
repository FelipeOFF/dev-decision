...
from dev_decision.claude_setup import (
    diagnose_claude,
    install_claude,
    uninstall_claude,
)
...
from dev_decision.cursor_setup import diagnose_cursor, install_cursor, uninstall_cursor
from dev_decision.grok_bot import (
    diagnose_grok_bot,
    install_grok_bot,
    uninstall_grok_bot,
)
from dev_decision.grok_build_setup import (
    diagnose_grok_build,
    install_grok_build,
...
_WORKFLOW_MARKER = ".dev-decision-public.json"
...
def _tree_digest(path: Path) -> dict[str, str]:
    return {
        ...
        if file.is_file() and file.name != _WORKFLOW_MARKER
    }


def _workflow_manifest(target: Path) -> dict[str, Any] | None:
    marker = target / _WORKFLOW_MARKER
...
def _owned_profile(profile: dict[str, Any] | None) -> bool:
    return bool(
        profile and profile.get("owner") == _OWNER and profile.get("profile") == PROFILE
    )


def _profile_paths(home: Path) -> tuple[Path, Path, Path]:
    root = home / ".config/dev-decision"
    return (
        root / "profiles/default.json",
        root / "credentials/default.key",
        home / ".local/bin/dev-decision-codex",
    )
...
def _wrapper_path(home: Path, harness: HarnessName) -> Path:
    ...
    return home / ".local/bin" / f"dev-decision-{suffix}"
...
def _wrapper_content(credential_path: Path, command: Sequence[str]) -> bytes:
    ...
    return (
        ...
        '  echo "Dev Decision MCP unavailable: credential is missing." >&2\n'
...
def _skill_targets(root: Path) -> list[Path]:
    return [root / "dev-decision", *(root / name for name in _WORKFLOW_SKILLS)]
...
def _managed_targets(
    ...
    for harness in harnesses:
        ...
        if harness == "codex":
            ...
            targets.extend(_skill_targets(home / ".dev-decision/grok-bot"))
    return targets
...
def _install_selected_harness(
    ...
    if harness == "cursor":
        return {
            "adapter": install_cursor(source, home / ".cursor/skills"),
            "workflows": _install_workflow_skills(
    ...
    if harness == "grok-build":
        return {
            "adapter": install_grok_build(source, home / ".grok/skills"),
            "workflows": _install_workflow_skills(source_root, home / ".grok/skills"),
        }
    bundle_root = home / ".dev-decision/grok-bot"
    return {
...
def install_public_harnesses(
    ...
    *,
    ...
    home = home or Path.home()
    source = source_root / "dev-decision"
    if not (source / "SKILL.md").is_file():
        raise ValueError("The Dev Decision skill must contain SKILL.md.")
    profile_path, credential_path, _ = _profile_paths(home)
    previous = _load_profile(profile_path)
    _validate_profile_files(profile_path, credential_path, previous)
    credential = f"{api_key}\n".encode()
    wrappers: dict[str, dict[str, str]] = {}
    mcp_created = False
...
def update_public_codex(
    ...
    *,
    ...
    target = home / ".agents/skills/dev-decision"
...
async def doctor_public_codex(
    ...
    *,
    ...
    home = home or Path.home()
    profile_path = home / ".config/dev-decision/profiles/default.json"
    profile = _load_profile(profile_path)
...
async def _sync_project_id(
    ...
    from dev_decision.identity_client import ProjectClient
...
def uninstall_public_harnesses(
    *,
    ...
    if enabled.get("grok-bot") is True:
        reports["grok-bot"] = {
            "adapter": uninstall_grok_bot(home / ".dev-decision/grok-bot"),
            "workflows": _uninstall_workflow_skills(
                home / ".dev-decision/grok-bot"
            ),
        }
    if enabled.get("codex") is True:
