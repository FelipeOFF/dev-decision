...
from dev_decision.protocol import (
    HANDSHAKE_TOOL,
    PROTOCOL_VERSION,
    NegotiatedProtocol,
    negotiate,
...
async def _session(
    ...
    *,
    ...
    if not token or any(not 33 <= ord(c) <= 126 for c in token):
        raise ValueError("Configure DEV_DECISION_MCP_API_KEY válida no cliente.")
                ...
                session = await stack.enter_async_context(
                    ClientSession(
                        ...
                        client_info=Implementation(
                            name="dev-decision-python", version=PROTOCOL_VERSION
                        ),
                    )
                )
